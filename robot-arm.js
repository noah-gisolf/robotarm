...
var RobotArm = function (canvas) {
     ...
     * @returns
     */
    self.moveRight = function (repeats) {
        // Don't do anything if we would move out of the columns
		for (i=1; i<=repeats; i++){
			if (local.state.arm.position + 1 >= self.columns) return;
     ...
     */
    self.moveLeft = function (repeats) {
		for (i=1; i<=repeats; i++){
	...
	/**
     * Moves blocks a certain columns to the right
     */
	self.grabAndDrop = function (distance) {
		//for (i=1; i<=repeats; i++){			this doesn't work somehow
			robotArm.grab();
			robotArm.moveRight(distance);
			robotArm.drop();
			robotArm.moveLeft(distance);
		//}
	}
	/**
     * Moves blocks a certain columns to the left
     */
	self.grabAndDrop2 = function (distance) {
			robotArm.grab();
			robotArm.moveLeft(distance);
			robotArm.drop();
			robotArm.moveRight(distance);
	...
	self.solve3 = function(){
	robotArm.loadLevel("exercise 3");
	robotArm.speed = 666;
	robotArm.grabAndDrop(2);
	robotArm.grabAndDrop(4);	
	robotArm.grabAndDrop(6);
	robotArm.grabAndDrop(8);
	}
	self.solve6  = function(){
	robotArm.loadLevel("exercise 6");
	robotArm.speed = 666;
	robotArm.moveRight(7);
	robotArm.grabAndDrop2(5);
	robotArm.moveLeft(1);
	robotArm.grabAndDrop2(6);
	robotArm.moveLeft(1);
	robotArm.grabAndDrop2(4);
	robotArm.moveLeft(1);
	robotArm.grabAndDrop2(1);
	robotArm.moveLeft(4);
	}
	self.solve9 = function(){
	robotArm.loadLevel("exercise 9");
	robotArm.speed = 666;
	robotArm.grabAndDrop(4);
	robotArm.moveRight(1);
	robotArm.grabAndDrop(4);
	robotArm.grabAndDrop(4);
	robotArm.moveRight(1);
	robotArm.grabAndDrop(4);
	...
	robotArm.moveRight(1);
	robotArm.grabAndDrop2(1);
	robotArm.grabAndDrop2(2);
	robotArm.grabAndDrop2(3);
	robotArm.moveRight(3);
	robotArm.grabAndDrop2(4);
	robotArm.grabAndDrop2(5);
	robotArm.grabAndDrop2(6);
	robotArm.moveLeft(1);
